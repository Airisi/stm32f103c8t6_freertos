...
//	OSIntEnter();    //UCOS的时候进出中断的时候需要添加OSIntEnter()和OSIntExit(),使用FreeRTOS就不需要
...
//串口4中断服务程序
//注意,读取USART4->SR能避免莫名其妙的错误   	
u8 USART4_RX_BUF[USART2_REC_LEN];     //接收缓冲,最大USART4_REC_LEN个字节.
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
//bit13~0，	接收到的有效字节数目
...
void USART4_Config(u32 bound)
{
	...
	// 将USART4 Tx的GPIO配置为推挽复用模式
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOC, &GPIO_InitStructure);

  // 将USART4 Rx的GPIO配置为浮空输入模式
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_11;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;
	GPIO_Init(GPIOC, &GPIO_InitStructure);
